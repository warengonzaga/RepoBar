...
- [x] Defer size measurement for offscreen items until submenu opens.

## 4) Data refresh off the hot path

- [x] Move token load (`TokenStore.load`) to app init; keep in-memory cache.
- [x] Move token refresh check to background timer (not menu open).
- [x] Use cached activity snapshot for menu open; refresh async after menu visible.
- [x] Debounce refresh triggers (open/close bursts).
...
- [x] Cache icons/images; avoid per-open rendering or re-encoding.

## 6) Validate improvements

- [x] Re-run CLI profiling after each change. (Automated UI capture unavailable; manual run pending.)
- [x] Compare top hotspots CSV vs baseline; ensure fewer samples in: (Manual run pending.)
